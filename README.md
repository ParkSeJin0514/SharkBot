...
| 커맨드 / 흐름 | 설명 | 상태 |
|---|---|---|
| **`/zendesk`** | 문의 폼(모달) 작성 → Zendesk 티켓 자동 생성(사진 첨부 포함), 티켓 번호 회신 | ✅ 운영 |
| **`/zendesk-status`** | 요청자(Slack 이메일) 기준으로 본인 티켓 목록·상태 조회 + **각 티켓에 `💬 답장` 버튼** | ✅ 운영 |
| **답장(티켓 이어쓰기)** | `💬 답장` 버튼 → 모달(**사진 첨부 가능**) → 새 티켓 없이 **같은 티켓에 공개 코멘트 추가** | ✅ 운영 |
| **사진/파일 첨부** | 모달 `file_input`(png/jpg/gif/pdf) → Zendesk 티켓에 첨부 | ✅ 운영 |
